...
# Procedimento para encontrar o número com maior módulo (valor absoluto)
def MaiorModulo(VetNum):
    ...
    if len(VetNum) == 1:
        ...
        return VetNum[0], abs(VetNum[0]), 0
...
for item in entrada:
    try:
        VetInicial.append(float(item.replace(",", ".")))  # converte cada item para float
    except ValueError:
        print(f"Erro: '{item}' não é um número válido. Ignorado.")
        # se algum item não for número, mostra erro e ignora
